...
def _get_detections(dataset, retinanet, score_threshold=0.05, max_detections=100, save_path=None):
    ...
    with torch.no_grad():
        ...
        for index in range(len(dataset)):
            ...
            # run network
            scores, labels, boxes = retinanet(data)
...
def _get_annotations(generator):
    ...
    for i in range(len(generator)):
        ...
        if len(annotations['labels']) == 0:
            annotations = np.array([[-1, -1, -1, -1, -1]])
        else:
            annotations = np.hstack((annotations['bboxes'], np.expand_dims(annotations['labels'], axis=1)))
...
def evaluate(
    ...
    print('Evaluating...\n Obtaining detections for evaluation set...')
    all_detections     = _get_detections(generator, retinanet, score_threshold=score_threshold, max_detections=max_detections, save_path=save_path)
    print('Evaluating...\n Obtaining annotations for evaluation set...')
    all_annotations    = _get_annotations(generator)
    print('Computing per-class mAP...')
